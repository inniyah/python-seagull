#!/usr/bin/env python
# See: https://groups.google.com/forum/#!msg/alt.sources/eRG2bL3Re-k/FvRLrRl0RiIJ
"""
...
def putNumber(num, length):
    ...
    for i in range(length):
        n = 8 * (length - 1 - i)
        lst.append(bytearray(((num >> n) & 0xFF,)))
    return b''.join(lst)
...
def putVariableLengthNumber(x):
    ...
    while 1:
        y, x = x & 0x7F, x >> 7
        lst.append(bytearray((y + 0x80,)))
        if x == 0:
            break
    lst.reverse()
    lst[-1] = bytearray((ord(lst[-1]) & 0x7f,))
    return b''.join(lst)
...
class MidiEvent:
    ...
    def write(self):
        ...
        if channelVoiceMessages.hasattr(self.type):
            ...
            str = bytearray((0xFF, getattr(metaEvents, self.type)))
            str = str + putVariableLengthNumber(len(self.data))
            return str + self.data
